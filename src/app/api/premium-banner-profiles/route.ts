...
const PREMIUM_BANNER_PLAN = "TOP PREMIUM BANNER";
...
export async function GET() {
  ...
  const [girls, trans] = await Promise.all([
    db
      .collection("girls")
      .find(publicVisibilityQuery, { projection: premiumBannerProjection })
      .sort({ createdAt: -1 })
      .limit(80)
      .toArray(),
    db
  ...
  const combined = [
    ...
    .filter((entry) => entry.premiumPlan === PREMIUM_BANNER_PLAN)
